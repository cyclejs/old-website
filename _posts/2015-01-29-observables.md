...
In the simplest case, the computer generates pixels on the screen, and the human generates click and keypress events. The computer observes these user inputs and the human observes the screen state generated by the computer. Notice that we can model each of these as *Observables*:

- Computer's output: a screen Observable.
- Human's output: an Observable of click/keypress events.
...
[Drivers](/drivers.html) are proxies to the user, and each driver represents one aspect of external effects. For instance, the DOM Driver takes a "screen" Observable generated by the computer, and returns Observables of click and keypress events. In between, the DOM Driver function produces "*write*" side-effects to render elements on the DOM, and catches "*read*" side-effects to detect user interaction. This way, the DOM Driver function can act on behalf of the user.
...
This is how the name "*Cycle.js*" came to be. It is a framework that solves the innate cyclic dependency of Observables which emerge during dialogues (mutual observations) between the Human and the Computer.
